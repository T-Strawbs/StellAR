...
public class ExplodableHandler : Singleton<ExplodableHandler>, CustomMessageHandler
{
    ...
    public void requestInteractableExplostion(Interactable interactable)
    {
    ...
    private IEnumerator requestInteractableExplostionCoroutine(Interactable interactable)
    {
        //request the Interactable Instance Manager to lockout this interactable across the network
    ...
    private void interactableExplosionClientBroadcast(ulong senderID, FastBufferReader messagePayload)
    {
        ...
        Interactable targetInteractable = (Interactable) NetworkInteractableInstanceManager.Instance.lookupNetworkInteractable
            (
    ...
    public void requestInteractableCollapse(Interactable interactable, bool isSingleCollapse)
    {
    ...
    private IEnumerator interactableCollaseServerRequest(Interactable interactable, bool isSingleCollapse)
    {
        //request that each iteractable which needs to be collapsed be locked
        List<Interactable> lockedInteractables = InteractableLockHandler.Instance.
        ...
        while(lockedCount != lockedInteractables.Count && timeoutDuration < GlobalConstants.MAX_WAIT_TIME)
        {
            ...
            foreach (Interactable lockedInteractable in lockedInteractables)
            {
