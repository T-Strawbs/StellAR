...
public class ExtendableObjectManipulator : StatefulInteractable
{
    #region Public Enums
    ...
    protected TransformFlags allowedManipulations = TransformFlags.Move | TransformFlags.Rotate | TransformFlags.Scale;
    ...
    protected InteractionFlags allowedInteractionTypes = InteractionFlags.Near | InteractionFlags.Ray | InteractionFlags.Gaze | InteractionFlags.Generic;
    ...
    protected bool applyTorque = true;
    ...
    protected float springForceSoftness = 0.1f;
    ...
    protected float springTorqueSoftness = 0.1f;
    ...
    protected float springDamping = 1.0f;
    ...
    protected float springForceLimit = 100.0f;
    ...
    protected RotateAnchorType rotationAnchorNear = RotateAnchorType.RotateAboutGrabPoint;
    ...
    protected RotateAnchorType rotationAnchorFar = RotateAnchorType.RotateAboutGrabPoint;
    ...
    protected SystemType transformSmoothingLogicType = typeof(DefaultTransformSmoothingLogic);
    ...
    protected bool smoothingFar = true;
    ...
    protected bool smoothingNear = true;
    ...
    protected float moveLerpTime = 0.001f;
    ...
    protected float rotateLerpTime = 0.001f;
    ...
    protected float scaleLerpTime = 0.001f;
    ...
    protected bool enableConstraints = true;
    ...
    protected ConstraintManager constraintsManager;
    ...
    protected LogicType manipulationLogicTypes = new LogicType
    {
    ...
    protected ITransformSmoothingLogic smoothingLogic;

    protected bool ShouldSmooth => (IsGrabSelected && SmoothingNear) || (!IsGrabSelected && SmoothingFar);

    protected Rigidbody rigidBody;

    protected bool wasGravity = false;

    protected bool wasKinematic = false;

    // Reusable list for fetching interactionPoints from interactors.
    protected List<Pose> interactionPoints = new List<Pose>();

    // Reusable list for fetching attachPoints from interactors.
    protected List<Pose> attachPoints = new List<Pose>();

    // Reusable list for fetching grabPoints from interactors.
    protected List<Pose> grabPoints = new List<Pose>();
    ...
    {
        // Manipulator is never selected by poking.
        DisableInteractorType(typeof(IPokeInteractor));
    }

    protected void InstantiateManipulationLogic()
    {
    ...
    protected InteractionFlags GetInteractionFlagsFromInteractor(IXRInteractor interactor)
    {
    ...
    protected Transform referenceFrameTransform = null;
    protected bool referenceFrameHasLastPos = false;
    protected Vector3 referenceFrameLastPos;

    protected MixedRealityTransform targetTransform;
    protected bool useForces;

    protected static readonly ProfilerMarker OnSelectEnteredPerfMarker =
        new ProfilerMarker("[MRTK] Manipulator.OnSelectEntered");
    ...
    protected static readonly ProfilerMarker OnSelectExitedPerfMarker =
        new ProfilerMarker("[MRTK] Manipulator.OnSelectExited");
    ...
    protected static readonly ProfilerMarker ScaleLogicMarker = new ProfilerMarker("[MRTK] ScaleLogic.Update");
    protected static readonly ProfilerMarker RotateLogicMarker = new ProfilerMarker("[MRTK] RotateLogic.Update");
    protected static readonly ProfilerMarker MoveLogicMarker = new ProfilerMarker("[MRTK] MoveLogic.Update");

    protected static readonly ProfilerMarker ObjectManipulatorProcessInteractableMarker =
        new ProfilerMarker("[MRTK] Manipulator.ProcessInteractable");
    ...
    /// </summary>
    protected virtual void ApplyTargetTransform()
    {
    ...
    /// </summary>
    protected virtual void ApplyForcesToRigidbody()
    {
    ...
    protected void ReleaseRigidBody(Vector3 velocity, Vector3 angularVelocity)
    {
        if (rigidBody != null)
        {
    ...
    /// </summary>
    protected bool TryGetGripRotation(IXRSelectInteractor interactor, out Quaternion rotation)
    {
...
public static class ReleaseBehaviorTypeExtensions
{
    ...
    public static bool IsMaskSet(this ExtendableObjectManipulator.ReleaseBehaviorType a, ExtendableObjectManipulator.ReleaseBehaviorType b)
    {
        return ((a & b) == b);
    }
}
